...
impl BasicConfig {
    ...
    pub fn default_from<P>(env: super::Environment, path: P) -> Result<Self>
        where P: AsRef<Path>
    {
        let mut config: BasicConfig = BasicConfig::default(env);

        let config_file_path = path.as_ref().to_path_buf();
        if let Some(parent) = config_file_path.parent() {
            config.set_root(parent);
        } else {
            let msg: &str = "Configuration files must be rooted in a directory.";
